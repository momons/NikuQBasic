	...
	while(true) {
		if (sym == "&&" ||
			...
			if (!QBasicValidation::isValidExpression(value, valueDist)) {
				interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
											  ErrorType::BadVariableType,
											  QBasicErrors::buildBadVariableType(value.types, valueDist.types));
			}
			...
			if (value.types[0] != VariableType::Bool) {
				interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
											  ErrorType::BadVariableType,
											  QBasicErrors::buildBadVariableType(value.types, {VariableType::Bool}));
			}
			...
			if (!QBasicValidation::isValidVariableType(value1, value2)) {
				interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
											  ErrorType::BadVariableType,
											  QBasicErrors::buildBadVariableType(value1.types, value2.types));
			}
...
QBasicVariableEntity QBasicCompile::addsub(const bool run) {
	...
	if (sym == "+") {
		...
		if (QBasicValidation::isValidSub(value)) {
			...
			interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Int, VariableType::Float}, value));
		}
	} else if(sym == "!") {
		interpreter->symbols->snapshot();
		value = muldiv(run);
		if (QBasicValidation::isValidNot(value)) {
			...
			interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Bool}, value));
		}
	} else {
		pushBack(sym);
		value = muldiv(run);
...
QBasicVariableEntity QBasicCompile::dictValue(const bool run) {
	...
	while (true) {
		...
		if (keyValue.types[0] != VariableType::Str) {
			interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Str}, keyValue));
		}
		match(":");
		QBasicVariableEntity value;
		sym = getSymbol();
	...
	while (true) {
		...
		if (variable->types[0] == VariableType::List) {
			if (keyValue.types[0] != VariableType::Int) {
				interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
											  ErrorType::BadVariableType,
											  QBasicErrors::buildBadVariableType({VariableType::Int}, keyValue));
				return nullptr;
			}
...
QBasicVariableEntity QBasicCompile::executeStatement(const bool run, const string &functionName, const bool needsReturnValue) {
	...
	if (entity->compileFunc != nullptr) {
		entity->compileFunc(interpreter, argList, interpreter->symbols->snapshotOffset());
	}
...
bool QBasicCompile::analysisValueAssigned(const bool run, const string &variableName) {
	...
	// 定数ならば
	if (variable->isConst) {
		interpreter->errors->addError(variableOffset - 2,
									  ErrorType::NoChangeConstValue,
									  variableName);
	}
	// 記号
	...
	if (!QBasicValidation::isValidVariableType(value, variable->types)) {
		interpreter->errors->addError(valueOffset,
									  ErrorType::BadVariableType,
									  QBasicErrors::buildBadVariableType({variable->types[0]}, value));
	}
	if (operate == "+=") {
		if (!QBasicValidation::isValidAdd(*variable, value)) {
			interpreter->errors->addError(operateOffset,
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Int, VariableType::Float, VariableType::Str}, value));
		}
	} else if (operate == "-=") {
		...
		if (!QBasicValidation::isValidMod(*variable, value)) {
			interpreter->errors->addError(operateOffset,
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Int}, value));
		}
	} else if (operate != "=") {
		interpreter->errors->addError(operateOffset,
...
bool QBasicCompile::analysisVar(const bool run, const bool isConst) {
	...
	if (!QBasicValidation::isValidVariableName(variableName)) {
		interpreter->errors->addError(variableOffset,
									  ErrorType::BadVariableName,
									  variableName);
	}
	...
	if (sym == ":") {
		...
		if (variableTypes[0] == VariableType::Unknown) {
			interpreter->errors->addError(offset,
										  ErrorType::UnknownSymbol,
										  sym);
		}
	...
	if (sym != "=") {
		...
		switch (variableTypes[0]) {
			case VariableType::List:
				variableEntity = QBasicVariableEntity(variableName,
													  variableTypes,
													  vector<QBasicVariableEntity>());
				break;
			case VariableType::Dict:
				variableEntity = QBasicVariableEntity(variableName,
													  variableTypes,
													  map<string, QBasicVariableEntity>());
				break;
			default:
				variableEntity = QBasicVariableEntity(variableName,
													  variableTypes,
	...
	if (variableTypes[0] != VariableType::Unknown) {
		if (!QBasicValidation::isValidVariableType(value, variableTypes)) {
			interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType(variableTypes, value));
		}
	} else {
		value.types = QBasicVariableEntity::getVariableTypes(value);
	}
...
bool QBasicCompile::analysisFor(const bool run) {
	...
	if (valueEntity == nullptr) {
		...
		if (!QBasicValidation::isValidFor(interpreter->variables[variableName])) {
			interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
										  ErrorType::BadVariableType,
										  QBasicErrors::buildBadVariableType({VariableType::Int}, *valueEntity));
		}
	}
...
bool QBasicCompile::analysisFunc(const bool run) {
	...
	if (!lastFunctionName.empty()) {
		interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
									  ErrorType::FunctionOverFunction,
									  lastFunctionName);
	}
...
bool QBasicCompile::analysisReturn(const bool run) {
	...
	if (!QBasicValidation::isValidVariableType(value, entity->returnTypes)) {
		interpreter->errors->addError(interpreter->symbols->snapshotOffset(),
									  ErrorType::BadVariableType,
									  QBasicErrors::buildBadVariableType(entity->returnTypes, value));
	}
