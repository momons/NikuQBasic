 ...
 *  @return 作成可否
 */
bool QBInterpreterScene::init() {

	// タッチ初期化
	initTouch();
	
	// 初期化
	setupNodes = map<int, QBInterpreterSetupNodeEntity>();
	...
	interpreter = new QBInterpreter(this,
									// 通信
...
void QBInterpreterScene::menuCloseCallback(Ref* pSender) {
 ...
 *  @param delta デルタ
 */
void QBInterpreterScene::update(float delta) {
 ...
 *  @param node  追加するnode
 */
void QBInterpreterScene::addNode(int index, QBInterpreterSetupNodeEntity node) {
	// 追加
 ...
 *  @return Entity
 */
QBInterpreterSetupNodeEntity * QBInterpreterScene::getSetupNode(int index) {
 ...
 *  ノードを設定
 */
void QBInterpreterScene::setupNode() {
	...
	for (auto it = setupNodes.begin();it != setupNodes.end();it++) {
		...
		for (auto it2 = setupNode->setupTypes.begin();it2 != setupNode->setupTypes.end();it2++) {
			...
			switch (setupType) {
				case QBInterpreterSetupType::Hidden:{
				...
				case QBInterpreterSetupType::Create:{
					createNode(index, setupNode);
					break;
				}
				case QBInterpreterSetupType::Move:
					moveNode(index);
					break;
				case QBInterpreterSetupType::MoveAdd:
					moveAddNode(index);
					break;
				case QBInterpreterSetupType::Scale:
				case QBInterpreterSetupType::ScaleAdd:
					scaleNode(index);
					break;
				case QBInterpreterSetupType::Rotation:
				case QBInterpreterSetupType::RotationAdd:
					rotationNode(index);
					break;
				case QBInterpreterSetupType::Mirror:
					break;
			}
			// 要素から削除
 ...
 *  @param node  ノード
 */
void QBInterpreterScene::createNode(int index, QBInterpreterSetupNodeEntity *node) {
	...
	// 表示設定
	switch (node->type) {
		case QBInterpreterSetupNodeType::Text: {
		...
		case QBInterpreterSetupNodeType::Image: {
			break;
		}
		case QBInterpreterSetupNodeType::Particle: {
			// 設定されていない
		...
		case QBInterpreterSetupNodeType::Line: {
		...
		case QBInterpreterSetupNodeType::Rect: {
			Vec2 points[] = {
				Vec2(node->x1, node->y1),
		...
		case QBInterpreterSetupNodeType::Circle: {
			vector<Vec2> points;
		...
		case QBInterpreterSetupNodeType::Triangle: {
			Vec2 points[] = {
				Vec2(node->x1, node->y1),
				Vec2(node->x2, node->y2),
 ...
 *  @param index     インデックス
 */
void QBInterpreterScene::moveNode(int index) {
 ...
 *  @param index     インデックス
 */
void QBInterpreterScene::moveAddNode(int index) {
 ...
 *  @param index     インデックス
 */
void QBInterpreterScene::scaleNode(int index) {
	...
	if (nodeEntity.type != QBInterpreterSetupNodeType::Text &&
		nodeEntity.type != QBInterpreterSetupNodeType::Image &&
		nodeEntity.type != QBInterpreterSetupNodeType::Particle) {
		setupNodes[index] = nodeEntity;
		// 位置設定
		setLocateNode(index);
	}
 ...
 *  @param index     インデックス
 */
void QBInterpreterScene::rotationNode(int index) {
 ...
 *  @param index     インデックス
 */
void QBInterpreterScene::setLocateNode(int index) {
 ...
 *  タッチ初期化
 */
void QBInterpreterScene::initTouch() {
 ...
 *  タッチ終了クリア処理
 */
void QBInterpreterScene::clearTouch() {
	// タッチ終了カウント
	if (touchEnd.size() > 0) {
		removeTouchEndCount++;
		// 0.5秒でクリア
